...
/* simple vector code */
/* STATUS: verified */
void mul_fvector_S(float to[3], const float from[3], float scalar)
{
    to[0] = from[0] * scalar;
    to[1] = from[1] * scalar;
    to[2] = from[2] * scalar;
...
void mul_fvectorT_fvector(float to[3][3], const float vectorA[3], const float vectorB[3])
{
...
/* STATUS: HAS TO BE verified (*should* work) */
void mul_fvectorT_fvectorS(float to[3][3], float vectorA[3], float vectorB[3], float aS)
{
...
/* create long vector */
lfVector* create_lfvector(uint verts)
{
...
/* delete long vector */
void del_lfvector(float(*fLongVector)[3])
{
    if (fLongVector != NULL) {
...
/* copy long vector */
void cp_lfvector(float(*to)[3], float(*from)[3], uint verts)
{
    memcpy(to, from, verts * sizeof(lfVector));
}
/* init long vector with float[3] */
void init_lfvector(float(*fLongVector)[3], const float vector[3], uint verts)
{
    uint i = 0;
    for (i = 0; i < verts; i++) {
...
void zero_lfvector(float(*to)[3], uint verts)
{
    memset(to, 0.0f, verts * sizeof(lfVector));
}
/* Multiply long vector with scalar. */
void mul_lfvectorS(float(*to)[3],
    float(*fLongVector)[3],
    float scalar,
    uint verts)
{
...
/* Multiply long vector with scalar.
 * `A -= B * float` */
void submul_lfvectorS(float(*to)[3],
    float(*fLongVector)[3],
    float scalar,
    uint verts)
{
...
float dot_lfvector(float(*fLongVectorA)[3],
...
void add_lfvector_lfvector(float(*to)[3],
...
void add_lfvector_lfvectorS(float(*to)[3],
    float(*fLongVectorA)[3],
    float(*fLongVectorB)[3],
    float bS,
    uint verts)
...
void add_lfvectorS_lfvectorS(float(*to)[3],
    float(*fLongVectorA)[3],
    float aS,
    float(*fLongVectorB)[3],
    float bS,
...
/* `A = B - C * float` -> for big vector. */
void sub_lfvector_lfvectorS(float(*to)[3],
    float(*fLongVectorA)[3],
    float(*fLongVectorB)[3],
    float bS,
    uint verts)
...
/* `A = B - C` -> for big vector. */
void sub_lfvector_lfvector(float(*to)[3],
    float(*fLongVectorA)[3],
    float(*fLongVectorB)[3],
    uint verts)
{
...
/* copy 3x3 matrix */
void cp_fmatrix(float to[3][3], const float from[3][3])
{
...
/* copy 3x3 matrix */
void initdiag_fmatrixS(float to[3][3], float aS)
{
    cp_fmatrix(to, ZERO);
...
float det_fmatrix(float m[3][3])
{
    return m[0][0] * m[1][1] * m[2][2] + m[1][0] * m[2][1] * m[0][2] + m[0][1] * m[1][2] * m[2][0] -
        m[0][0] * m[1][2] * m[2][1] - m[0][1] * m[1][0] * m[2][2] - m[2][0] * m[1][1] * m[0][2];
}

void inverse_fmatrix(float to[3][3], float from[3][3])
{
    uint i, j;
    float d;
...
/* STATUS: verified */
void mul_fmatrix_S(float matrix[3][3], float scalar)
{
...
/* STATUS: verified */
void mul_fvector_fmatrix(float* to, const float* from, const float matrix[3][3])
{
...
/* STATUS: verified */
void mul_fmatrix_fvector(float* to, const float matrix[3][3], const float from[3])
{
...
void add_fmatrix_fmatrix(float to[3][3],
...
void sub_fmatrix_fmatrix(float to[3][3],
    const float matrixA[3][3],
    const float matrixB[3][3])
{
...
/* STATUS: verified */
void muladd_fmatrix_fvector(float to[3], const float matrix[3][3], const float from[3])
{
    to[0] += dot_v3v3(matrix[0], from);
    to[1] += dot_v3v3(matrix[1], from);
    to[2] += dot_v3v3(matrix[2], from);
...
void muladd_fmatrixT_fvector(float to[3], const float matrix[3][3], const float from[3])
{
    to[0] += matrix[0][0] * from[0] + matrix[1][0] * from[1] + matrix[2][0] * from[2];
    to[1] += matrix[0][1] * from[0] + matrix[1][1] * from[1] + matrix[2][1] * from[2];
    to[2] += matrix[0][2] * from[0] + matrix[1][2] * from[1] + matrix[2][2] * from[2];
...
void outerproduct(float r[3][3], const float a[3], const float b[3])
{
...
void cross_m3_v3m3(float r[3][3], const float v[3], const float m[3][3])
{
...
void cross_v3_identity(float r[3][3], const float v[3])
{
    r[0][0] = 0.0f;
    r[1][0] = v[2];
    r[2][0] = -v[1];
...
void init_fmatrix(fmatrix3x3* matrix, int r, int c)
{
    matrix->r = r;
    matrix->c = c;
}
...
/* create big matrix */
fmatrix3x3* create_bfmatrix(uint verts, uint springs)
{
    /* TODO: check if memory allocation was successful */
    auto* temp = (fmatrix3x3*)MEM_callocN(sizeof(fmatrix3x3) * (verts + springs), "cloth_implicit_alloc_matrix");
...
/* delete big matrix */
void del_bfmatrix(fmatrix3x3* matrix)
{
    if (matrix != NULL) 
    {
        MEM_freeN(matrix);
...
/* copy big matrix */
void cp_bfmatrix(fmatrix3x3* to, fmatrix3x3* from)
{
...
/* init big matrix */
/* slow in parallel */
void init_bfmatrix(fmatrix3x3* matrix, float m3[3][3])
{
    uint i;
...
/* slow in parallel */
void initdiag_bfmatrix(fmatrix3x3* matrix, float m3[3][3])
{
    uint i, j;
...
/* STATUS: verified */
void mul_bfmatrix_lfvector(float(*to)[3], fmatrix3x3* from, lfVector* fLongVector)
{
...
/* VERIFIED */
void subadd_bfmatrixS_bfmatrixS(
...
void world_to_root_v3(Implicit_Data* data, int index, float r[3], const float v[3])
{
    copy_v3_v3(r, v);
    mul_transposed_m3_v3(data->tfm[index].m, r);
}

void root_to_world_v3(Implicit_Data* data, int index, float r[3], const float v[3])
{
    mul_v3_m3v3(r, data->tfm[index].m, v);
}

void world_to_root_m3(Implicit_Data* data,
    int index,
    float r[3][3],
    const float m[3][3])
{
...
void root_to_world_m3(Implicit_Data* data,
    int index,
    float r[3][3],
    const float m[3][3])
{
...
void filter(lfVector* V, fmatrix3x3* S)
{
    uint i = 0;
...
void dfdx_spring(float to[3][3], const float dir[3], float length, float L, float k)
{
...
void dfdv_damp(float to[3][3], const float dir[3], float damping)
{
...
float fbderiv(float length, float L)
{
    float x = length / L;
    float xx = x * x;
    float xxx = xx * x;
...
float fbstar(float length, float L, float kb, float cb)
{
...
float fbstar_jacobi(float length, float L, float kb, float cb)
{
    float tempfb_fl = kb * fb(length, L);
    float fbstar_fl = cb * (length - L);
...
bool spring_length(Implicit_Data* data,
    int i,
    int j,
    float r_extent[3],
    float r_dir[3],
...
void apply_spring(Implicit_Data* data,
    int i,
    int j,
    const float f[3],
...
void poly_avg(lfVector* data, const int* inds, int len, float r_avg[3])
{
...
void poly_norm(lfVector* data, int i, int j, int* inds, int len, float r_dir[3])
{
    float mid[3];
...
void edge_avg(lfVector* data, int i, int j, float r_avg[3])
{
...
void edge_norm(lfVector* data, int i, int j, float r_dir[3])
{
    sub_v3_v3v3(r_dir, data[i], data[j]);
    normalize_v3(r_dir);
}

float bend_angle(const float dir_a[3], const float dir_b[3], const float dir_e[3])
{
    float cos, sin;
    float tmp[3];
...
void spring_angle(Implicit_Data* data,
    int i,
    int j,
    int* i_a,
    int* i_b,
...
void spring_hairbend_forces(Implicit_Data* data,
    int i,
    int j,
    int k,
    const float goal[3],
...
void spring_hairbend_estimate_dfdx(Implicit_Data* data,
    int i,
    int j,
    int k,
    const float goal[3],
...
/* Finite Differences method for estimating the jacobian of the force */
void spring_hairbend_estimate_dfdv(Implicit_Data* data,
    int i,
    int j,
    int k,
    const float goal[3],
