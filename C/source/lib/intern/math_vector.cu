...
__global__ void g_interp_v3_v3v3(float r[3], const float a[3], const float b[3], const float t)
{
    const float s = 1.0f - t;
...
bool interp_v3_v3v3_slerp(float target[3], const float a[3], const float b[3], const float t)
{
  ...
  const float cosom = dot_v3v3(a, b);
...
bool interp_v2_v2v2_slerp(float target[2], const float a[2], const float b[2], const float t)
{
  ...
  const float cosom = dot_v2v2(a, b);
...
__host__ __device__ bool is_finite_v2(const float v[2])
{
...
__host__ __device__ void axis_sort_v3(const float axis_values[3], int r_axis_order[3])
{
  float v[3];
