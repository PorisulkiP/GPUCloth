...
__host__ __device__ static void *stack_get_last_elem(const BLI_Stack *stack)
{
  return static_cast<char*>((stack)->chunk_curr->data) + ((stack)->elem_size * (stack)->chunk_index);
}

/**
 * \return number of elements per chunk, optimized for slop-space.
 */
__host__ __device__ static size_t stack_chunk_elem_max_calc(const size_t elem_size, size_t chunk_size)
{
...
__host__ __device__ BLI_Stack *BLI_stack_new_ex(const size_t elem_size,
                            const char *description,
                            const size_t chunk_size)
{
    BLI_Stack*stack = static_cast<BLI_Stack*>(MEM_lockfree_callocN(sizeof(*stack), description));
 ...
 */
__host__ __device__ BLI_Stack *BLI_stack_new(const size_t elem_size, const char *description)
{
  return BLI_stack_new_ex(elem_size, description, CHUNK_SIZE_DEFAULT);
}

__host__ __device__ static void stack_free_chunks(struct StackChunk *data)
{
  while (data) {
 ...
 */
__host__ __device__ void BLI_stack_free(BLI_Stack *stack)
{
	stack_free_chunks(stack->chunk_curr);
	stack_free_chunks(stack->chunk_free);
	MEM_lockfree_freeN(stack);
}
 ...
 */
__host__ __device__ void *BLI_stack_push_r(BLI_Stack *stack)
{
  stack->chunk_index++;
  ...
  if (UNLIKELY(stack->chunk_index == stack->chunk_elem_max)) {
    ...
    if (stack->chunk_free) {
      ...
      chunk = (StackChunk*)MEM_lockfree_mallocN(sizeof(*chunk) + (stack->elem_size * stack->chunk_elem_max), __func__);
    }
 ...
 */
__host__ __device__ void BLI_stack_push(BLI_Stack *stack, const void *src)
{
 ...
 */
__host__ __device__ void BLI_stack_pop(BLI_Stack *stack, void *dst)
{
 ...
 */
__host__ __device__ void BLI_stack_pop_n(BLI_Stack *stack, void *dst, uint n)
{
 ...
 */
__host__ __device__ void BLI_stack_pop_n_reverse(BLI_Stack *stack, void *dst, uint n)
{
  BLI_assert(n <= BLI_stack_count(stack));
...
__host__ __device__ void *BLI_stack_peek(BLI_Stack *stack)
{
  BLI_assert(BLI_stack_is_empty(stack) == false);
 ...
 */
__host__ __device__ void BLI_stack_discard(BLI_Stack *stack)
{
  BLI_assert(BLI_stack_is_empty(stack) == false);
 ...
 */
__host__ __device__ void BLI_stack_clear(BLI_Stack *stack)
{
#ifdef USE_TOTELEM
...
__host__ __device__ size_t BLI_stack_count(const BLI_Stack *stack)
{
#ifdef USE_TOTELEM
 ...
 */
__host__ __device__ bool BLI_stack_is_empty(const BLI_Stack *stack)
{
#ifdef USE_TOTELEM
