...
void BLI_parallel_range_settings_defaults(TaskParallelSettings *settings);
__device__ inline void d_BLI_parallel_range_settings_defaults(TaskParallelSettings* settings);

void BLI_task_parallel_range(int start, int stop, void *userdata, TaskParallelRangeFunc func, const TaskParallelSettings *settings);
__device__ void d_BLI_task_parallel_range(const int start, const int stop, void* userdata, const TaskParallelRangeFunc func, const TaskParallelSettings* settings);
...
inline void BLI_parallel_range_settings_defaults(TaskParallelSettings *settings)
{
...
__device__ inline void d_BLI_parallel_range_settings_defaults(TaskParallelSettings* settings)
{
    cudaMemset(settings, 0, sizeof(*settings));
    settings->use_threading = true;
    /* Use default heuristic to define actual chunk size. */
    settings->min_iter_per_thread = 0;
}

inline void BLI_parallel_mempool_settings_defaults(TaskParallelSettings *settings)
{
  memset(settings, 0, sizeof(*settings));
  settings->use_threading = true;
}

__device__ inline void d_BLI_parallel_mempool_settings_defaults(TaskParallelSettings* settings)
{
    cudaMemset(settings, 0, sizeof(*settings));
    settings->use_threading = true;
}
 ...
 *
 * \code{.c}
 * MyData *task_data = MEM_lockfree_callocN(sizeof(MyData), __func__);
 * TaskNode *root = BLI_task_graph_node_create(task_graph, root_exec, task_data, MEM_lockfree_freeN);
