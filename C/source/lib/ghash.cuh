...
#include "sys_types.cuh" /* for bool */
...
#define _GHASH_INTERNAL_ATTR
    ...
    typedef struct GHashIterState {
        uint curr_bucket _GHASH_INTERNAL_ATTR;
    } GHashIterState;
    ...
    GHash* BLI_ghash_new_ex(GHashHashFP hashfp,
        GHashCmpFP cmpfp,
        const char* info,
        uint nentries_reserve) ATTR_MALLOC ATTR_WARN_UNUSED_RESULT;
    /**
     ...
     */
    void BLI_ghash_reserve(GHash* gh, uint nentries_reserve);
    /**
     ...
     */
    uint BLI_ghash_len(const GHash* gh) ATTR_WARN_UNUSED_RESULT;
    /**
     * Sets a GHash flag.
     */
    ...
    __host__ __device__ void* BLI_ghashIterator_getValue(GHashIterator* ghi);
    void** BLI_ghashIterator_getValue_p(GHashIterator* ghi);
    bool BLI_ghashIterator_done(const GHashIterator* ghi);

    struct _gh_Entry {
        void* next, * key, * val;
    };
    inline void* BLI_ghashIterator_getKey(GHashIterator* ghi)
    {
        return reinterpret_cast<struct _gh_Entry*>(ghi->curEntry)->key;
    }
    __host__ __device__ inline void* BLI_ghashIterator_getValue(GHashIterator* ghi)
    {
        return reinterpret_cast<struct _gh_Entry*>(ghi->curEntry)->val;
    }
    inline void** BLI_ghashIterator_getValue_p(GHashIterator* ghi)
    {
        return &reinterpret_cast<struct _gh_Entry*>(ghi->curEntry)->val;
    }
    inline bool BLI_ghashIterator_done(const GHashIterator* ghi)
    {
        return !ghi->curEntry;
    }
 ...
 * \{ */
    ...
    uint BLI_gset_len(const GSet* gs) ATTR_WARN_UNUSED_RESULT;
    ...
    void BLI_gset_clear_ex(GSet* gs, GSetKeyFreeFP keyfreefp, uint nentries_reserve);
    ...
    inline GSetIterator* BLI_gsetIterator_new(GSet* gs)
    {
        return (GSetIterator*)BLI_ghashIterator_new((GHash*)gs);
    }
    inline void BLI_gsetIterator_init(GSetIterator* gsi, GSet* gs)
    {
        BLI_ghashIterator_init((GHashIterator*)gsi, (GHash*)gs);
    }
    inline void BLI_gsetIterator_free(GSetIterator* gsi)
    {
        BLI_ghashIterator_free((GHashIterator*)gsi);
    }
    inline void* BLI_gsetIterator_getKey(GSetIterator* gsi)
    {
        return BLI_ghashIterator_getKey((GHashIterator*)gsi);
    }
    inline void BLI_gsetIterator_step(GSetIterator* gsi)
    {
        BLI_ghashIterator_step((GHashIterator*)gsi);
    }
    inline bool BLI_gsetIterator_done(const GSetIterator* gsi)
    {
...
#define BLI_ghashutil_inthash(key) \
  (CHECK_TYPE_ANY(&(key), int *, const int *), BLI_ghashutil_uinthash((uint)key))
...
#define BLI_ghashutil_inthash_v4(key) \
  (CHECK_TYPE_ANY(key, int *, const int *), BLI_ghashutil_uinthash_v4((const uint *)key))
...
#define BLI_ghashutil_inthash_v4_murmur(key) \
  (CHECK_TYPE_ANY(key, int *, const int *), \
   BLI_ghashutil_uinthash_v4_murmur((const uint *)key))
