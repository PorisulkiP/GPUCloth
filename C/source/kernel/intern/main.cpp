...
//      to_id_entry->session_uuid = (*id_pointer != NULL) ? (*id_pointer)->session_uuid : MAIN_ID_SESSION_UUID_UNSET;
...
void BKE_main_relations_create(Main *bmain, const short flag)
{
  ...
  //    const int idwalk_flag = IDWALK_READONLY | ((flag & MAINIDRELATIONS_INCLUDE_UI) != 0 ? IDWALK_INCLUDE_UI : 0);
                                ...
                                const bool value)
{
  ...
  for (gh_iter = BLI_ghashIterator_new(bmain->relations->relations_from_pointers);
    ...
    MainIDRelationsEntry *entry = (MainIDRelationsEntry*)BLI_ghashIterator_getValue(gh_iter);
    if (value) {
      entry->tags |= tag;
    }
    else {
