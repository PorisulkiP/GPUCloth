...
bool BKE_shrinkwrap_init_tree( ShrinkwrapTreeData *data, Mesh *mesh, int shrinkType, int shrinkMode, bool force_normals)
{
  ...
  if (force_normals || BKE_shrinkwrap_needs_normals(shrinkType, shrinkMode)) {
    data->pnors = (float(*)[3])CustomData_get_layer(&mesh->pdata, CD_NORMAL);
    if ((mesh->flag & ME_AUTOSMOOTH) != 0) {
      data->clnors = (float(*)[3])CustomData_get_layer(&mesh->ldata, CD_NORMAL);
    }
  }
...
static ShrinkwrapBoundaryData *shrinkwrap_build_boundary_data(struct Mesh *mesh)
{
  ...
  //BLI_bitmap *looptri_has_boundary = (BLI_bitmap*)BLI_BITMAP_NEW(totlooptri,"ShrinkwrapBoundaryData::looptri_is_boundary");
                                                 ...
                                                 const TaskParallelTLS *__restrict tls)
{
  ...
  float tmp_co[3];
  float weight = 0;// BKE_defvert_array_find_weight_safe(calc->dvert, i, calc->vgroup);
                               ...
                               int numVerts)
{
  ...
  if (smd->target != NULL) 
  {
    Object* ob_target = nullptr;// DEG_get_evaluated_object(ctx->depsgraph, smd->target);
