...
static void ptcache_particle_extra_read(void* psys_v, PTCacheMem * pm)
{
    ...
    for (; extra; extra = extra->next) {
        ...
        case BPHYS_EXTRA_FLUID_SPRINGS: {
            ...
            psys->fluid_springs = (ParticleSpring*)MEM_dupallocN(extra->data);
...
static void ptcache_cloth_interpolate(int index, void* cloth_v,  void** data,
    float cfra, float cfra1, float cfra2, const float* old_data)
{
    ClothModifierData* clmd = (ClothModifierData*)cloth_v;
    Cloth* cloth = clmd->clothObject;
    ClothVertex* vert = cloth->verts + index;
...
static void ptcache_add_extra_data(PTCacheMem* pm,
    ...
    void* data)
{
    PTCacheExtra* extra = (PTCacheExtra*)MEM_callocN(sizeof(PTCacheExtra), "Point cache: extra data descriptor");
...
static void ptcache_particle_extra_write(void* psys_v, PTCacheMem* pm)
{
    ...
    if (psys->part->phystype == PART_PHYS_FLUID && psys->part->fluid &&
        ...
        psys->fluid_springs)
    {
        ptcache_add_extra_data(pm, BPHYS_EXTRA_FLUID_SPRINGS, psys->tot_fluidsprings, psys->fluid_springs);
    }
}
...
static void ptcache_cloth_extra_read(void* cloth_v, PTCacheMem* pm)
{
    ...
    for (; extra; extra = extra->next) {
        ...
        case BPHYS_EXTRA_CLOTH_ACCELERATION: {
            copy_v3_v3(cloth->average_acceleration, (const float*)extra->data);
            break;
        }}
    }
}
...
int BKE_ptcache_id_reset(Scene* scene, PTCacheID* pid, int mode)
{
    ...
    if (reset) {
        ...
        if (pid->type == PTCACHE_TYPE_CLOTH) {
            cloth_free_modifier((ClothModifierData*)pid->calldata);
        }
...
int BKE_ptcache_object_reset(Scene* scene, Object* ob, int mode)
{
    ...
    //        FluidDomainSettings* fds = (FluidDomainSettings*)fmd->domain;
...
void BKE_ptcache_id_clear(PTCacheID* pid, int mode, uint cfra)
{
    ...
    if (!pid || !pid->cache || pid->cache->flag & PTCACHE_BAKED) {return;}
    if (pid->cache->flag & PTCACHE_IGNORE_CLEAR) {return;}
    ...
    case PTCACHE_CLEAR_AFTER:
        if (pid->cache->flag & PTCACHE_DISK_CACHE) {
            ...
            PTCacheMem* pm = nullptr;// (PTCacheMem*)pid->cache->mem_cache.first;
...
void BKE_ptcache_id_time(PTCacheID* pid, Scene* scene, float cfra, int* startframe, int* endframe, float* timescale)
{
    ...
    if (timescale) {
        time = scene->r.subframe * scene->r.framelen;
        nexttime = (scene->r.cfra + 1 + scene->r.subframe) * scene->r.framelen;
    ...
    if (cache->cached_frames == NULL && cache->endframe > cache->startframe) {
        ...
        cache->cached_frames = (char*)MEM_callocN(sizeof(char) * cache->cached_frames_len, "cached frames array");
...
//  char new_path_full[255];
//  char old_path_full[255];
//  char ext[255];
//
//  /* save old name */
