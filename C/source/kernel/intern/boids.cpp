...
{
  const float *normal = (float*)user_data;
  float d[3], dist;
...
static bool rule_goal_avoid(BoidRule *rule, BoidBrainData *bbd, BoidValues *val, ParticleData *pa)
{
  ...
  if (effectors) {
    for (cur = (EffectorCache*)effectors->first; cur; cur = cur->next) {
      Object *eob = cur->ob;
...
ParticleSystem* psys_get_target_system(Object* ob, ParticleTarget* pt)
{
    ...
    if (ELEM(pt->ob, NULL, ob)) {
        psys = (ParticleSystem*)BLI_findlink(&ob->particlesystem, pt->psys - 1);
    }
    else {
        psys = (ParticleSystem*)BLI_findlink(&pt->ob->particlesystem, pt->psys - 1);
    }
                                 ...
                                 ParticleData *pa)
{
  ...
  if (acbr->options & BRULE_ACOLL_WITH_DEFLECTORS && bbd->sim->colliders) {
    ...
    for (coll = (ColliderCache*)bbd->sim->colliders->first; coll; coll = coll->next) {
  ...
  for (pt = (ParticleTarget*)bbd->sim->psys->targets.first; pt; pt = pt->next) {
                          ...
                          ParticleData *pa)
{
  ...
  int neighbors = 0;// BLI_kdtree_3d_range_search(bbd->sim->psys->tree, pa->prev_state.co, &ptn, 2.0f * val->personal_space * pa->size);
  bool ret = false;
  ...
  for (pt = (ParticleTarget*)bbd->sim->psys->targets.first; pt; pt = pt->next) {
    ...
    if (epsys) {
      //neighbors = BLI_kdtree_3d_range_search( epsys->tree, pa->prev_state.co, &ptn, 2.0f * val->personal_space * pa->size);
                       ...
                       ParticleData *pa)
{
  ...
  float vec[3] = {0.0f, 0.0f, 0.0f}, loc[3] = {0.0f, 0.0f, 0.0f};
  int neighbors = 0;// BLI_kdtree_3d_find_nearest_n_with_len_squared_cb(
  //    bbd->sim->psys->tree,
  //    pa->state.co,
  //    ptn,
  //    ARRAY_SIZE(ptn),
...
static bool rule_fight(BoidRule *rule, BoidBrainData *bbd, BoidValues *val, ParticleData *pa)
{
  ...
  int neighbors = 0;// BLI_kdtree_3d_range_search(bbd->sim->psys->tree, pa->prev_state.co, &ptn, fbr->distance);
  for (n = 0; n < neighbors; n++) {
...
{
  BoidState *state = (BoidState*)part->boids->states.first;
  BoidRule *rule;
...
void boid_brain(BoidBrainData *bbd, int p, ParticleData *pa)
{
  ...
  switch (state->ruleset_type) {
    ...
    case eBoidRulesetType_Random: {
      ...
      if (n) {
        rule = (BoidRule*)BLI_findlink(&state->rules, rand % n);
...
void boid_body(BoidBrainData *bbd, ParticleData *pa)
{
  ...
  pd_point_from_particle(bbd->sim, pa, &pa->state, &epoint);
  effectors_apply(bbd->sim->psys->effectors,
                      bbd->sim->colliders,
...
BoidRule *boid_new_rule(int type)
{
  ...
  //strncpy(rule->name, rna_enum_boidrule_type_items[type - 1].name, sizeof(rule->name));
...
BoidState *boid_new_state(BoidSettings *boids)
{
  ...
  if (state->id) {
    printf(state->name, sizeof(state->name), "State %i", state->id);
  }
  else {
...
BoidSettings *boid_copy_settings(const BoidSettings *boids)
{
  ...
  if (boids) {
    ...
    state = (BoidState*)boids->states.first;
    nstate = (BoidState*)nboids->states.first;
