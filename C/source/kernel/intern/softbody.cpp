                                 ...
                                 int *UNUSED(ptr_to_break_func(void)))
{
  ...
  ListBase *effectors = effectors_create(
  ...
  for (bb = number_of_points_here; bb > 0; bb--, bp++) {
    ...
    if (_final_goal(ob, bp) < SOFTGOALSNAP) { /* omit this bp when it snaps */
      ...
      if (effectors) {
        ...
        effectors_apply(effectors, NULL, sb->effector_weights, &epoint, force, NULL, speed);
    ...
    struct Depsgraph *depsgraph, Scene *scene, Object *ob, float forcetime, float timenow)
{
  ...
  ListBase *effectors = effectors_create(depsgraph, ob, NULL, sb->effector_weights, false);
...
SoftBody *sbNew(Scene *scene)
{
  ...
  if (!sb->effector_weights) {
    sb->effector_weights = effector_add_weights(NULL);
  }
