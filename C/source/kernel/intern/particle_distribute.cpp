...
static int psys_thread_context_init_distribute(ParticleThreadContext* ctx, ParticleSimulationData* sim, int from)
{
    ...
    const bool use_render_params = (sim->depsgraph->mode == DAG_EVAL_RENDER);
    ...
    if (from == PART_FROM_CHILD) {
        ...
        if (part->distr == PART_DISTR_GRID && from != PART_FROM_VERT) {
            if (psys->part->use_modifier_stack) {
                ...
                //mesh = (Mesh*)BKE_id_copy_ex(NULL, (ID*)ob->data, NULL, LIB_ID_COPY_LOCALIZE);
            }
    ...
    if (from == PART_FROM_CHILD) {
        ...
        if (from == PART_FROM_VERT) {
            MVert* mv = mesh->mvert;
            float(*orcodata)[3];// = (float(*)[3])CustomData_get_layer(&mesh->vdata, CD_ORCO);
    ...
    if ((part->flag & PART_EDISTR || children) && from != PART_FROM_VERT) {
        ...
        //orcodata = (float(*)[3])CustomData_get_layer(&mesh->vdata, CD_ORCO);
    ...
    if (part->type == PART_HAIR && !(part->childtype == PART_CHILD_FACES && part->parents != 0.0f)) {
        ...
        //if (from == PART_FROM_VERT) {
        //    if (mesh->totvert) {
        //        orig_index = (int*)CustomData_get_layer(&mesh->vdata, CD_ORIGINDEX);
        //    }
        //}
        //else
        //{
        //    if (mesh->totface)
        //    {
        //        orig_index = (int*)CustomData_get_layer(&mesh->fdata, CD_ORIGINDEX);
        //    }
        //}
