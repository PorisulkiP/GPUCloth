...
EffectorWeights* effector_add_weights(Collection* collection)
{
...
PartDeflect* BKE_partdeflect_copy(const struct PartDeflect* pd_src)
{
    ...
    PartDeflect* pd_dst = (PartDeflect*)MEM_dupallocN(pd_src);
    if (pd_dst->rng != NULL) {
...
static void add_effector_relation(ListBase* relations,
    ...
    PartDeflect* pd)
{
    EffectorRelation* relation = (EffectorRelation*)MEM_callocN(sizeof(EffectorRelation), "EffectorRelation");
    relation->ob = ob;
    relation->psys = psys;
    relation->pd = pd;
...
{
    if (*effectors == NULL) {
        *effectors = (ListBase*)MEM_callocN(sizeof(ListBase), "effector effectors");
    }

    EffectorCache* eff = (EffectorCache*)MEM_callocN(sizeof(EffectorCache), "EffectorCache");
...
//ListBase* effector_relations_create(Depsgraph* depsgraph,
...
void effector_relations_free(ListBase* lb)
{
    if (lb) {
        BLI_freelistN(lb);
        MEM_freeN(lb);
    ...
    bool use_rotation)
{
    ...
    ListBase* relations = nullptr; //DEG_get_effector_relations(depsgraph, weights->group);
    ...
    LISTBASE_FOREACH(EffectorRelation*, relation, relations) {
        ...
        /* Получение оцениваемого объекта. */
        Object* ob = (Object*)DEG_get_evaluated_id(depsgraph, &relation->ob->id);

        if (relation->psys) 
        {
            /* Get evaluated particle system. */
            ParticleSystem* psys = (ParticleSystem*)BLI_findstring( &ob->particlesystem, relation->psys->name, offsetof(ParticleSystem, name));
            ParticleSettings* part = psys->part;
            ...
            PartDeflect* pd = nullptr; //(relation->pd == relation->psys->part->pd) ? part->pd : part->pd2;
...
void effectors_free(ListBase* lb)
{
    if (lb) {
    ...
    EffectedPoint* point)
{
    ...
    for (col = (ColliderCache*)colls->first; col; col = col->next) {
...
bool get_effector_data(struct EffectorCache* eff,
    ...
    int real_velocity)
{
    float cfra = eff->depsgraph->ctime;
    bool ret = false;
    ...
    //    const float(*vert_normals)[3] = me_eval;
    //    if (me_eval != NULL) {
...
void effectors_apply(ListBase* effectors,
    ListBase* colliders,
    EffectorWeights* weights,
    EffectedPoint* point,
    float* force,
...
{
    const SimDebugElement* elem = (SimDebugElement*)key;
    return elem->hash;
}

static bool debug_element_compare(const void* a, const void* b)
{
    const SimDebugElement* elem1 = (SimDebugElement*)a;
    const SimDebugElement* elem2 = (SimDebugElement*)b;
...
{
    SimDebugElement* elem = (SimDebugElement*)val;
    MEM_freeN(elem);
}
...
void BKE_sim_debug_data_set_enabled(bool enable)
{
    if (enable) {
        if (!_sim_debug_data) {
            _sim_debug_data = (SimDebugData*)MEM_callocN(sizeof(SimDebugData), "sim debug data");
...
{
    SimDebugElement* old_elem = (SimDebugElement*)BLI_ghash_lookup(debug_data->gh, elem);
    if (old_elem) {
        *old_elem = *elem;
        MEM_freeN(elem);
    }
    ...
    uint hash)
{
    ...
    elem = (SimDebugElement*)MEM_callocN(sizeof(SimDebugElement), "sim debug data element");
    elem->type = type;
    ...
    if (str) {
        strncpy(elem->str, str, sizeof(elem->str));
    }
    else {
        elem->str[0] = '\0';
    }
