...
static void idp_repr_fn_recursive(struct ReprState *state, const ID *prop)
{
...
#define STR_APPEND_FMT(format, ...) \
  ...
  switch (prop->type) {
    ...
    case IDP_IDPARRAY: {
      STR_APPEND_STR("[");
      for (const ID *v = prop->data.pointer, *v_end = v + prop->len; v != v_end; v++) {
        if (v != prop->data.pointer) {
          STR_APPEND_STR(", ");
        }
    ...
    case IDP_GROUP: {
      STR_APPEND_STR("{");
      LISTBASE_FOREACH (const ID *, subprop, &prop->data.group) {
...
void IDP_repr_fn(const ID *prop,
                 void (*str_append_fn)(void *user_data, const char *str, uint str_len),
                 void *user_data)
{
...
char *IDP_reprN(const ID *prop, uint *r_len)
{
