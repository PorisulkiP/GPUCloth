  ...
  if (id) {
    ...
    if (id->us <= limit) {
      if (GS(id->name) != ID_IP) {
        ...
        //           id->name, id->lib ? id->lib->filepath_abs : "[Main]",
        //           id->us, limit);
        BLI_assert(0);
      }
      id->us = limit;
...
// * \param flag: Set of copy options, see ID.h enum for details (leave to zero for default,
// *              full copy).
...
//  }
//
//  ID *id = (ID*)BKE_libblock_alloc(bmain, type, name, 0);
