...
/* IDTemplate is a union in ID.h */

/**
 * if the new is 'IDP_ARRAY_REALLOC_LIMIT' items less,
 * than #ID.totallen, reallocate anyway.
 */
 ...
 */
ID *IDP_NewIDPArray(const char *name)
{
  ID *prop = MEM_callocN(sizeof(ID), "ID prop array");
...
ID *IDP_CopyIDPArray(const ID *array, const int flag)
{
  ...
  *narray = *array;
  ...
  for (int i = 0; i < narray->len; i++) {
     ...
     * in this loop.*/
    ID *tmp = IDP_CopyProperty_ex(GETPROP(narray, i), flag);
    memcpy(GETPROP(narray, i), tmp, sizeof(ID));
    MEM_freeN(tmp);
  }
...
static void IDP_FreeIDPArray(ID *prop, const bool do_id_user)
{
...
/* shallow copies item */
void IDP_SetIndexArray(ID *prop, int index, ID *item)
{
  BLI_assert(prop->type == IDP_IDPARRAY);
  ...
  ID *old = GETPROP(prop, index);
  if (item != old) {
    IDP_FreePropertyContent(old);

    memcpy(old, item, sizeof(ID));
  }
}

ID *IDP_GetIndexArray(ID *prop, int index)
{
  BLI_assert(prop->type == IDP_IDPARRAY);
...
void IDP_AppendArray(ID *prop, ID *item)
{
  BLI_assert(prop->type == IDP_IDPARRAY);
...
void IDP_ResizeIDPArray(ID *prop, int newlen)
{
  BLI_assert(prop->type == IDP_IDPARRAY);
  ...
  prop->data.pointer = MEM_recallocN(prop->data.pointer, sizeof(ID) * (size_t)newsize);
  prop->len = newlen;
  prop->totallen = newsize;
}

/* ----------- Numerical Array Type ----------- */
static void idp_resize_group_array(ID *prop, int newlen, void *newarr)
{
...
void IDP_ResizeArray(ID *prop, int newlen)
{
...
static ID *idp_generic_copy(const ID *prop, const int UNUSED(flag))
{
  ID *newp = MEM_callocN(sizeof(ID), __func__);
...
static ID *IDP_CopyArray(const ID *prop, const int flag)
{
  ID *newp = idp_generic_copy(prop, flag);

  if (prop->data.pointer) {
    newp->data.pointer = MEM_dupallocN(prop->data.pointer);

    if (prop->type == IDP_GROUP) {
      ID **array = newp->data.pointer;
      int a;
...
/* -------------------------------------------------------------------- */
/** \name String Functions (ID String API)
 * \{ */
 ...
 */
ID *IDP_NewString(const char *st, const char *name, int maxlen)
{
  ID *prop = MEM_callocN(sizeof(ID), "ID string");
...
static ID *IDP_CopyString(const ID *prop, const int flag)
{
...
void IDP_AssignString(ID *prop, const char *st, int maxlen)
{
  BLI_assert(prop->type == IDP_STRING);
...
void IDP_ConcatStringC(ID *prop, const char *st)
{
  BLI_assert(prop->type == IDP_STRING);
...
void IDP_ConcatString(ID *str1, ID *append)
{
...
/* -------------------------------------------------------------------- */
/** \name ID Type (ID ID API)
 * \{ */

static ID *IDP_CopyID(const ID *prop, const int flag)
{
...
void IDP_AssignID(ID *prop, ID *id, const int flag)
{
  BLI_assert(prop->type == IDP_ID);
...
/* -------------------------------------------------------------------- */
/** \name Group Functions (ID Group API)
 * \{ */

/**
 * Checks if a property with the same name as prop exists, and if so replaces it.
 */
static ID *IDP_CopyGroup(const ID *prop, const int flag)
{
  ...
  LISTBASE_FOREACH (ID *, link, &prop->data.group) {
...
void IDP_SyncGroupValues(ID *dest, const ID *src)
{
  BLI_assert(dest->type == IDP_GROUP);
  BLI_assert(src->type == IDP_GROUP);

  LISTBASE_FOREACH (ID *, prop, &src->data.group) {
    ID *other = BLI_findstring(&dest->data.group, prop->name, offsetof(ID, name));
...
void IDP_SyncGroupTypes(ID *dest, const ID *src, const bool do_arraylen)
{
  LISTBASE_FOREACH_MUTABLE (ID *, prop_dst, &src->data.group) {
    const ID *prop_src = IDP_GetPropertyFromGroup((ID *)src, prop_dst->name);
    if (prop_src != NULL) {
 ...
 */
void IDP_ReplaceGroupInGroup(ID *dest, const ID *src)
{
  BLI_assert(dest->type == IDP_GROUP);
  BLI_assert(src->type == IDP_GROUP);
 ...
 */
void IDP_ReplaceInGroup_ex(ID *group, ID *prop, ID *prop_exist)
{
...
void IDP_ReplaceInGroup(ID *group, ID *prop)
{
  ID *prop_exist = IDP_GetPropertyFromGroup(group, prop->name);
 ...
 * Do it recursively.
 */
void IDP_MergeGroup_ex(ID *dest,
                       const ID *src,
                       ...
                       const int flag)
{
  ...
  if (do_overwrite) {
    LISTBASE_FOREACH (ID *, prop, &src->data.group) {
      if (prop->type == IDP_GROUP) {
        ID *prop_exist = IDP_GetPropertyFromGroup(dest, prop->name);
      ...
      ID *copy = IDP_CopyProperty_ex(prop, flag);
 ...
 * Do it recursively.
 */
void IDP_MergeGroup(ID *dest, const ID *src, const bool do_overwrite)
{
 ...
 */
bool IDP_AddToGroup(ID *group, ID *prop)
{
  BLI_assert(group->type == IDP_GROUP);
 ...
 */
bool IDP_InsertToGroup(ID *group, ID *previous, ID *pnew)
{
  BLI_assert(group->type == IDP_GROUP);
 ...
 */
void IDP_RemoveFromGroup(ID *group, ID *prop)
{
  BLI_assert(group->type == IDP_GROUP);
 ...
 */
void IDP_FreeFromGroup(ID *group, ID *prop)
{
  IDP_RemoveFromGroup(group, prop);
  IDP_FreeProperty(prop);
}

ID *IDP_GetPropertyFromGroup(const ID *prop, const char *name)
{
  BLI_assert(prop->type == IDP_GROUP);

  return (ID *)BLI_findstring(&prop->data.group, name, offsetof(ID, name));
}
/** same as above but ensure type match */
ID *IDP_GetPropertyTypeFromGroup(const ID *prop, const char *name, const char type)
{
  ID *idprop = IDP_GetPropertyFromGroup(prop, name);
 ...
 * direct data. */
static void IDP_FreeGroup(ID *prop, const bool do_id_user)
{
  BLI_assert(prop->type == IDP_GROUP);

  LISTBASE_FOREACH (ID *, loop, &prop->data.group) {
...
/* -------------------------------------------------------------------- */
/** \name Main Functions  (ID Main API)
 * \{ */

ID *IDP_CopyProperty_ex(const ID *prop, const int flag)
{
  switch (prop->type) {
    case IDP_GROUP:
...
ID *IDP_CopyProperty(const ID *prop)
{
  return IDP_CopyProperty_ex(prop, 0);
}

/**
 * Copy content from source ID into destination one, freeing destination property's content
 * first.
 */
void IDP_CopyPropertyContent(ID *dst, ID *src)
{
  ID *idprop_tmp = IDP_CopyProperty(src);
  idprop_tmp->prev = dst->prev;
  idprop_tmp->next = dst->next;
  SWAP(ID, *dst, *idprop_tmp);
 ...
 */
ID *IDP_GetProperties(ID *id, const bool create_if_needed)
{
  if (id->properties) {
  ...
  if (create_if_needed) {
    id->properties = MEM_callocN(sizeof(ID), "ID");
...
bool IDP_EqualsProperties_ex(ID *prop1, ID *prop2, const bool is_strict)
{
  ...
  switch (prop1->type) {
    ...
    case IDP_GROUP: {
      ...
      LISTBASE_FOREACH (ID *, link1, &prop1->data.group) {
        ID *link2 = IDP_GetPropertyFromGroup(prop2, link1->name);
    ...
    case IDP_IDPARRAY: {
      ID *array1 = IDP_IDPArray(prop1);
      ID *array2 = IDP_IDPArray(prop2);
...
bool IDP_EqualsProperties(ID *prop1, ID *prop2)
{
 ...
 * a memory leak.
 */
ID *IDP_New(const char type, const IDTemplate *val, const char *name)
{
  ID *prop = NULL;

  switch (type) {
    case IDP_INT:
      prop = MEM_callocN(sizeof(ID), "ID int");
      prop->data.val = val->i;
      break;
    case IDP_FLOAT:
      prop = MEM_callocN(sizeof(ID), "ID float");
      *(float *)&prop->data.val = val->f;
      break;
    case IDP_DOUBLE:
      prop = MEM_callocN(sizeof(ID), "ID double");
    ...
    case IDP_ARRAY: {
      ...
      if ((val->array.type == IDP_FLOAT) || (val->array.type == IDP_INT) ||
        ...
        prop = MEM_callocN(sizeof(ID), "ID array");
    ...
    case IDP_STRING: {
      ...
      prop = MEM_callocN(sizeof(ID), "ID string");
    ...
    case IDP_GROUP: {
      ...
      prop = MEM_callocN(sizeof(ID), "ID group");
      break;
    }
    case IDP_ID: {
      prop = MEM_callocN(sizeof(ID), "ID datablock");
 ...
 * But it does not free the actual ID struct itself.
 */
void IDP_FreePropertyContent_ex(ID *prop, const bool do_id_user)
{
  switch (prop->type) {
    case IDP_ARRAY:
      IDP_FreeArray(prop);
...
void IDP_FreePropertyContent(ID *prop)
{
  IDP_FreePropertyContent_ex(prop, true);
}

void IDP_FreeProperty_ex(ID *prop, const bool do_id_user)
{
  IDP_FreePropertyContent_ex(prop, do_id_user);
  MEM_freeN(prop);
}

void IDP_FreeProperty(ID *prop)
{
  IDP_FreePropertyContent(prop);
  MEM_freeN(prop);
}

void IDP_ClearProperty(ID *prop)
{
  IDP_FreePropertyContent(prop);
...
void IDP_Reset(ID *prop, const ID *reference)
{
  if (prop == NULL) {
    return;
  }
 ...
 * IDP_TYPE_FILTER_ enum in ID.h.
 */
void IDP_foreach_property(ID *id_property_root,
                          ...
                          void *user_data)
{
  ...
  switch (id_property_root->type) {
    case IDP_GROUP: {
      LISTBASE_FOREACH (ID *, loop, &id_property_root->data.group) {
        IDP_foreach_property(loop, type_filter, callback, user_data);
      }
      break;
    }
    case IDP_IDPARRAY: {
      ID *loop = IDP_Array(id_property_root);
...
void IDP_WriteProperty_OnlyData(const ID *prop, BlendWriter *writer);

static void IDP_WriteArray(const ID *prop, BlendWriter *writer)
{
...
static void IDP_WriteIDPArray(const ID *prop, BlendWriter *writer)
{
  /*REMEMBER to set totalen to len in the linking code!!*/
  if (prop->data.pointer) {
    const ID *array = prop->data.pointer;

    BLO_write_struct_array(writer, ID, prop->len, array);
...
static void IDP_WriteString(const ID *prop, BlendWriter *writer)
{
  /*REMEMBER to set totalen to len in the linking code!!*/
  BLO_write_raw(writer, (size_t)prop->len, prop->data.pointer);
}

static void IDP_WriteGroup(const ID *prop, BlendWriter *writer)
{
  LISTBASE_FOREACH (ID *, loop, &prop->data.group) {
...
void IDP_BlendWrite(BlendWriter *writer, const ID *prop)
{
  BLO_write_struct(writer, ID, prop);
  IDP_WriteProperty_OnlyData(prop, writer);
}

static void IDP_DirectLinkProperty(ID *prop, BlendDataReader *reader);

static void IDP_DirectLinkIDPArray(ID *prop, BlendDataReader *reader)
{
  /* since we didn't save the extra buffer, set totallen to len */
  prop->totallen = prop->len;
  BLO_read_data_address(reader, &prop->data.pointer);

  ID *array = (ID *)prop->data.pointer;
...
static void IDP_DirectLinkArray(ID *prop, BlendDataReader *reader)
{
  /* since we didn't save the extra buffer, set totallen to len */
  prop->totallen = prop->len;
...
static void IDP_DirectLinkString(ID *prop, BlendDataReader *reader)
{
...
static void IDP_DirectLinkGroup(ID *prop, BlendDataReader *reader)
{
...
static void IDP_DirectLinkProperty(ID *prop, BlendDataReader *reader)
{
  switch (prop->type) {
    ...
    case IDP_DOUBLE:
       ...
       * They are stored in the same field as `int val, val2` in the #IDData struct,
    ...
    default:
      ...
      printf(
          "%s: found unknown ID type %d, reset to Integer one !\n", __func__, prop->type);
...
void IDP_BlendReadData_impl(BlendDataReader *reader, ID **prop, const char *caller_func_id)
{
  if (*prop) {
    if ((*prop)->type == IDP_GROUP) {
      IDP_DirectLinkGroup(*prop, reader);
...
void IDP_BlendReadLib(BlendLibReader *reader, ID *prop)
{
  if (!prop) {
    return;
  }
  ...
  switch (prop->type) {
    ...
    {
      ID *idp_array = IDP_IDPArray(prop);
      for (int i = 0; i < prop->len; i++) {
...
void IDP_BlendReadExpand(struct BlendExpander *expander, ID *prop)
{
  if (!prop) {
    return;
  }
