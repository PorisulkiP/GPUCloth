...
#include "MEM_guardedalloc.cuh" /* for MEM_freeN MEM_mallocN MEM_callocN */
...
static bool init_structDNA(SDNA *sdna, bool do_endian_swap, const char **r_error_message)
{
  ...
  if (*data == MAKE_ID('N', 'A', 'M', 'E')) {
    ...
    data++;
    sdna->names = static_cast<const char**>(MEM_callocN(sizeof(void *) * sdna->names_len, "sdnanames"));
  }
  else {
  ...
  if (*data == MAKE_ID('T', 'Y', 'P', 'E')) {
    ...
    data++;
    sdna->types = static_cast<const char**>(MEM_callocN(sizeof(void *) * sdna->types_len, "sdnatypes"));
  }
  else {
  ...
  if (*data == MAKE_ID('S', 'T', 'R', 'C')) {
    ...
    data++;
    sdna->structs = static_cast<SDNA_Struct**>(MEM_callocN(sizeof(SDNA_Struct *) * sdna->structs_len, "sdnastrcs"));
  }
  else {
  ...
  /* Cache name size. */
  {
    short *names_array_len = static_cast<short*>(MEM_mallocN(sizeof(*names_array_len) * sdna->names_len, __func__));
                             ...
                             const void *old_blocks)
{
  ...
  reconstruct_structs(
      reconstruct_info, blocks, old_struct_nr, new_struct_nr, static_cast<const char*>(old_blocks), new_blocks);
  return new_blocks;
}
  ...
  switch (new_category) {
    case STRUCT_MEMBER_CATEGORY_STRUCT: {
      if (STREQ(new_type_name, old_type_name)) {
        ...
        enum eSDNA_StructCompare compare_flag = static_cast<eSDNA_StructCompare>(compare_flags[old_struct_nr]);
...
{
  ReconstructStep *steps = static_cast<ReconstructStep*>(MEM_calloc_arrayN(
...
DNA_ReconstructInfo *DNA_reconstruct_info_create(const SDNA *oldsdna,
                                                 ...
                                                 const char *compare_flags)
{
  DNA_ReconstructInfo *reconstruct_info = static_cast<DNA_ReconstructInfo*>(MEM_callocN(sizeof(DNA_ReconstructInfo), __func__));
  reconstruct_info->oldsdna = oldsdna;
  reconstruct_info->newsdna = newsdna;
  reconstruct_info->compare_flags = compare_flags;
  reconstruct_info->step_counts = (int*)MEM_malloc_arrayN(sizeof(int), newsdna->structs_len, __func__);
  reconstruct_info->steps = (ReconstructStep**)MEM_malloc_arrayN(
  ...
  for (int elem_index = sp->members_len; elem_index > 0; elem_index--) {
    ...
    if (DNA_elem_id_match(elem_old, elem_old_len, elem_old_full, &elem_old_full_offset_start)) {
      ...
      if (sdna->names_len == sdna->names_len_alloc) {
        ...
        sdna->names = static_cast<const char**>(MEM_recallocN((void *)sdna->names,
                                    sizeof(*sdna->names) * sdna->names_len_alloc));
        sdna->names_array_len = static_cast<short*>(MEM_recallocN(
...
static void sdna_expand_names(SDNA *sdna)
{
  ...
  const char **names_expand = static_cast<const char**>(MEM_mallocN(sizeof(*names_expand) * names_expand_len, __func__));
  short *names_array_len_expand = static_cast<short*>(MEM_mallocN(sizeof(*names_array_len_expand) * names_expand_len,  __func__));
  ...
  for (int struct_nr = 0; struct_nr < sdna->structs_len; struct_nr++) {
    ...
    SDNA_Struct *struct_new = (SDNA_Struct*)BLI_memarena_alloc(sdna->mem_arena, array_size);
                                                        ...
                                                        const char *elem_static_full)
{
  ...
  char *elem_static = (char*)malloc(elem_static_full_len + 1);
  ...
  const char *elem_alias = (const char*)BLI_ghash_lookup(elem_map_alias_from_static, str_pair);
  if (elem_alias) {
...
void DNA_sdna_alias_data_ensure(SDNA *sdna)
{
  ...
  if (sdna->alias.types == NULL) {
    sdna->alias.types = static_cast<const char**>(MEM_mallocN(sizeof(*sdna->alias.types) * sdna->types_len, __func__));
  ...
  if (sdna->alias.names == NULL) {
    ...
    sdna->alias.names = static_cast<const char **>(MEM_mallocN(sizeof(*sdna->alias.names) * sdna->names_len, __func__));
    for (int struct_nr = 0; struct_nr < sdna->structs_len; struct_nr++) {
      const SDNA_Struct *struct_info = sdna->structs[struct_nr];
